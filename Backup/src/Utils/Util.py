...
"""
# @Author           : Albert Wang
# @Time             : 2022-03-28 22:56:18
# @Description      :
# @Email            : shadowofgost@outlook.com
# @FilePath         : /ComputerScienceThesis/src/Utils/Util.py
# @LastTime         : 2022-03-28 23:55:24
...
def push_ios_release(tokens, message_type, content):
    ...
    thePayLoad = {
        ...
        "sbs": {"message_type": message_type},
    }
    # Certificate issued by apple and converted to .pem format with openSSL
    # theCertfile = './Development_v3.pem'
    print(".....")
    theCertfile = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "Development_v3.pem"
    )
    print("=-----", theCertfile)
    #
    ...
    ssl_sock = ssl.wrap_socket(
        socket.socket(socket.AF_INET, socket.SOCK_STREAM), certfile=theCertfile
    )
...
def push_ios_test(tokens, message_type, content):
    ...
    # Certificate issued by apple and converted to .pem format with openSSL
    theCertfile = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "Development_test.pem"
    )
    print(theCertfile)
    #
    # theHost = ( 'gateway.push.apple.com', 2195 )
...
def isWindows():
    return platform_system.lower().find("windows") >= 0
...
def SendSMSByZA(mobile_phone, content):
    q_value = "&mobile=%s&content=%s&sendTime=&extno=" % (
        urllib.quote(mobile_phone.encode("utf-8")),
        urllib.quote(content.encode("utf-8")),
    )
    url_send = (
        "http://www.591duanxin.com/sms.aspx?action=send&userid=539&account=rongshi106&password=123456%s"
        % q_value
    )
    print(url_send)
