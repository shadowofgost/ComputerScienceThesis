...
# @FilePath         : /ComputerScienceThesis/src/DataBase/orm.py
# @LastTime         : 2022-03-29 16:16:12
...
class Bulletinimage(db.Model):
    ...
    bulletin = db.relationship(
        "Bulletin", backref=db.backref("bulletinimages", cascade="all, delete-orphan")
    )
...
class InstitutionFeature(db.Model):
    ...
    institution = db.relationship(
        "Institution",
        backref=db.backref("institutionfeatures", cascade="all, delete-orphan"),
    )
...
class SchoolFeature(db.Model):
    ...
    school = db.relationship(
        "School", backref=db.backref("schoolfeatures", cascade="all, delete-orphan")
    )
    feature = db.relationship("Feature")
...
class Schoolimage(db.Model):
    ...
    school = db.relationship(
        "School", backref=db.backref("schoolimages", cascade="all, delete-orphan")
    )
